...
const VICTIM_TYPES = ['All', 'Civilians', 'Security Forces', 'Combatants']
    ...
    <div className="flex h-full flex-col gap-5">
      ...
      <div className="border-t border-border pt-4">
        ...
        <div className="flex flex-col gap-2.5">
          ...
          <select value={filters.victimType || 'All'} onChange={e => onChange('victimType', e.target.value)} className={`${filterStyle} ${chevronStyle}`}>
            {VICTIM_TYPES.map(v => <option key={v} value={v}>{v === 'All' ? 'Victim Type' : v}</option>)}
          </select>
        </div>
      </div>
    </div>
